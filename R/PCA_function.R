...
PCA<-function(data,stream=TRUE,nb_fact,principal_factors=TRUE,principal_axes=FALSE,eigenvalues=FALSE,corr=FALSE,graphics=FALSE,data_init,exec_time,print_step)
...
if (stream==TRUE) {
 ...
 init <- PCA_iter(data[1:data_init,],stream,nb_fact,principal_factors,principal_axes,eigenvalues,corr,exec_time) 
...
resultat<-PCA_iter(data,stream,nb_fact,principal_factors,principal_axes,eigenvalues,corr,exec_time)
